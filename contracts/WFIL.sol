...
contract WFIL is IWFIL {
    ...
    string public constant name = "Wrapped Filecoin";
    string public constant symbol = "WFIL";
    ...
    /// @dev Records amount of WFIL token owned by account.
    ...
    /// @dev Records number of WFIL token that account (second) will be allowed to spend on behalf of another account (first) through {transferFrom}.
    mapping (address => mapping (address => uint256)) public override allowance;

    /// @dev Current amount of flash-minted WFIL token.
    ...
    /// @dev Returns the total supply of WFIL token as the FIL held in this contract.
    function totalSupply() external view override returns (uint256) {
        return address(this).balance + flashMinted;
    }

    /// @dev Fallback, `msg.value` of FIL sent to this contract grants caller account a matching increase in WFIL token balance.
    ...
    /// @dev Return the amount of WFIL token that can be flash-lent.
    function maxFlashLoan(address token) external view override returns (uint256) {
        return token == address(this) ? type(uint112).max - flashMinted : 0; // Can't underflow
    }

    /// @dev Return the fee (zero) for flash lending an amount of WFIL token.
    function flashFee(address token, uint256) external view override returns (uint256) {
        require(token == address(this), "WFIL: flash mint only WFIL");
        return 0;
    }

    /// @dev Flash lends `value` WFIL token to the receiver address.
    /// By the end of the transaction, `value` WFIL token will be burned from the receiver.
    /// The flash-minted WFIL token is not backed by real FIL, but can be withdrawn as such up to the FIL balance of this contract.
    ...
    function flashLoan(IERC3156FlashBorrower receiver, address token, uint256 value, bytes calldata data) external override returns (bool) {
        require(token == address(this), "WFIL: flash mint only WFIL");
        require(value <= type(uint112).max, "WFIL: individual loan limit exceeded");
        flashMinted = flashMinted + value;
        require(flashMinted <= type(uint112).max, "WFIL: total loan limit exceeded");
        ...
        require(balance >= value, "WFIL: burn amount exceeds balance");
    ...
    function withdraw(uint256 value) external override {
        ...
        require(success, "WFIL: FIL transfer failed");
    }
    ...
    function withdrawFrom(address from, address payable to, uint256 value) external override {
        ...
        // _transferEther(to, value);
        (bool success, ) = to.call{value: value}("");
        require(success, "WFIL: Ether transfer failed");
    }
    ...
    /// @dev Sets `value` as allowance of `spender` account over `owner` account's WFIL token, given `owner` account's signed approval.
    /// Emits {Approval} event.
    /// Requirements:
    ...
    /// WFIL token implementation adapted from https://github.com/albertocuestacanada/ERC20Permit/blob/master/contracts/ERC20Permit.sol.
    function permit(address owner, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external override {
        require(block.timestamp <= deadline, "WFIL: Expired permit");
        ...
        require(signer != address(0) && signer == owner, "WFIL: invalid permit");
    ...
    function transfer(address to, uint256 value) external override returns (bool) {
        ...
        if (to != address(0) && to != address(this)) { // Transfer
            uint256 balance = balanceOf[msg.sender];
            require(balance >= value, "WFIL: transfer amount exceeds balance");
    ...
    /// @dev Moves `value` WFIL token from account (`from`) to account (`to`) using allowance mechanism.
